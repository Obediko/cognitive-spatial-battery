...
assert.doesNotMatch(sessions, /Only actively ongoing sessions can be deleted here/);
assert.match(sessions, /action:\s*"delete_session"/);
assert.match(sessions, /await artifacts\(\)\.delete\(item\.key\)/);
