...
Status: Draft v0.2 for expert review and pilot testing
Battery: ETI Core
...
3. Digits are presented one at a time via standardized audio, targeting a one-second onset-to-onset interval, with observed browser start times recorded, regardless of an individual digit clip's own duration.
...
## 4. Controlled sequence form

The task uses fixed, investigator-controlled `ons-controlled-form-a-1.0` arrays: two items at every forward length 3–9 and backward length 2–8. The same arrays are used for each participant, and defensive copies prevent runtime mutation. The form was constructed to avoid adjacent repetitions and obvious ascending/descending runs.

The form is an experimental item set, not a copied published worksheet. Its content is retained in research exports for auditability and should be access-controlled. Creating, reordering, or randomizing items requires a new `ns_sequence_version` and targeted validation.
...
Changes to the instruction wording, onset interval, starting/maximum length, discontinue rule, or controlled sequences increment `ns_task_version`; any item change also increments `ns_sequence_version`. Both are stored in research output.
